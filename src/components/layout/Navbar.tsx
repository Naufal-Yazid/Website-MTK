    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isSolid ? "bg-white shadow-sm border-b border-gray-200/80 py-0" : "bg-transparent border-b border-transparent py-1"}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-3 group">
          ...
          <span className={`text-lg tracking-tight transition-colors ${useDarkContent ? "text-[#111827] font-normal" : "text-white font-normal"}`}>Marga Tirta Kencana</span>
        </Link>
        ...
        <nav className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => {
            ...
            return (
              <Link key={link.href} href={link.href} className={`relative py-2 text-sm font-normal transition-colors ${active ? "text-[#0B5EAA]" : useDarkContent ? "text-[#4B5563] hover:text-[#0B5EAA]" : "text-white/90 hover:text-[#0B5EAA]"}`}>
                {link.label}
        ...
        <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className={`md:hidden p-2 rounded-lg transition-colors ${useDarkContent ? "text-gray-700 hover:bg-gray-100" : "text-white hover:bg-white/10"}`} aria-label="Toggle Navigation Menu">
